...
import utils
from tqdm.auto import tqdm  # Используем tqdm.auto для автоматического выбора версии
...
def main(args):
    ...
    for epoch in range(hp.epochs):
        ...
        training_loader = DataLoader(dataset,
                                     ...
                                     drop_last=True,
                                     num_workers=0)
        total_steps_in_epoch = len(training_loader) * hp.batch_size

        # Initialize progress bar for the epoch
        progress_bar = tqdm(total=total_steps_in_epoch,
        ...
        for i, batchs in enumerate(training_loader):
            for j, data_of_batch in enumerate(batchs):
                ...
                avg_loss_epoch = total_loss_epoch / num_steps_epoch

                # Update progress bar
                progress_bar.update(1)
                progress_bar.set_postfix_str(
                    f'AvgL: {avg_loss_epoch:.4f} | LastL: {t_l:.4f} | LR: {scheduled_optim.get_learning_rate():.6f}'
                )
                ...
                if current_step % hp.save_step == 0:
                    ...
                    print(f"\nSave model at step {current_step} ...")
        ...
        print(
            f"\nEpoch [{epoch+1}/{hp.epochs}] completed. Average Loss: {avg_loss_epoch:.4f}\n")
