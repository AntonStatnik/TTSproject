...
class Tacotron2Loss_LPC(nn.Module):
    ...
    def forward(self, model_outputs, targets):
        ...
        energy_loss   = self.loss(energy, energy_target) 
        cepstral_loss = self.loss(cepstral, cepstral_target) 
        pitch_loss    = self.loss(pitch, pitch_target) 

        energy_postnet_loss   = self.loss(energy_postnet, energy_target) 
        cepstral_postnet_loss = self.loss(cepstral_postnet, cepstral_target) 
        pitch_postnet_loss    = self.loss(pitch_postnet, pitch_target) 
